...
$status = !empty($_POST['status']) ? $_POST['status'] : (\Ketongu\Init\Article::STATUS_PENDING);

if (!empty($_POST['username'])) {
    $username = $_POST['username'];

    $article = new \Ketongu\Init\Article();
    $article
        ->setTitle($username)
        ->setSlug('zezeg')
        ->setStatus($status)
...
<body>
    <form action="" method="POST">
        ...
        <select name="status" id="status">
            <option value="<?php echo \Ketongu\Init\Article::STATUS_PENDING; ?>">En attente</option>
